    ...
    init: function(path, onready) {
      ...
      if (jsonrpc_interface.indexOf("http") == 0) {
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "Websocket 错误，你可能需要重新载入此页面以重启。");
          ws_callback = {};
        };
    ...
    add_task: function(uri, options) {
      ...
      ARIA2.request("addUri", [uri, options],
        ...
        function(result) {
          ...
          $("#add-task-alert").show();
          console.warn("add task 错误: "+error_msg);
        });
    },
    ...
    tell_active: function(keys) {
      ...
      ARIA2.request("tellActive", keys,
        function(result) {
          ...
          if (select_lock) return;
          if (!result.result) {
            main_alert("alert-error", "<strong>错误: </strong>rpc result error.", 5000);
          }
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已经暂停所有任务，请等待特殊完成任务（如连接种子服务器）。", 2000);
        }
      );
    },
    ...
    unpause_all: function() {
      ARIA2.request("unpauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "回复所有任务", 2000);
        }
      );
    },
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "移除所有已完成任务", 2000);
        }
      );
    },
